...
class HierAttnNet(nn.Module):
    ...
    def __init__(
        ...
        #self.speaker_embedding = nn.Embedding(4, speaker_embedding_dim, padding_idx=padding_idx)
        self.ld = nn.Dropout(p=last_drop)
        self.fc = nn.Linear((sent_hidden_dim * 2) + speaker_embedding_dim + 2 + 6, num_class)
        #self.fc = nn.Linear(sent_hidden_dim * 2 + speaker_embedding_dim + 2, num_class)

    def forward(self, X, S, T, totald, nttc, nttd, otc, otd, talkd):
        x = X.permute(1, 0, 2)
        #s = self.speaker_embedding(torch.LongTensor(S))
        ...
        doc_a, doc_s = self.sentattnnet(sent_s, S, T)
        doc_s = self.ld(doc_s)
        totald = F.normalize(torch.unsqueeze(totald, 1), p=2, dim=1)
        nttc = torch.unsqueeze(nttc, 1)
        nttd = torch.unsqueeze(nttd, 1)
        otc = torch.unsqueeze(otc, 1)
        otd = torch.unsqueeze(otd, 1)
        talkd = torch.unsqueeze(talkd, 1)
        doc_s = torch.cat([doc_s, totald, nttc, nttd, otc, otd, talkd], 1)
...
class RNNAttn(nn.Module):
    ...
    def __init__(
        ...
        self.ld = nn.Dropout(p=last_drop)
        self.fc = nn.Linear(hidden_dim * 2 + 6, num_class)
    ...
    def forward(self, X, S, T, totald, nttc, nttd, otc, otd, talkd):
        ...
        otd = torch.unsqueeze(otd, 1)
        talkd = torch.unsqueeze(talkd, 1)
        out = torch.cat([out, totald, nttc, nttd, otc, otd, talkd], 1)
        return self.fc(out)
...
class SentAttnNet(nn.Module):
    ...
    def __init__(
        self, word_hidden_dim=32, sent_hidden_dim=32, padding_idx=1, weight_drop=0.0, speaker_embedding_dim=8
    ):
        ...
        self.speaker_embedding = nn.Embedding(4, speaker_embedding_dim, padding_idx=padding_idx)
        self.sent_attn = AttentionWithContext(sent_hidden_dim * 2 + speaker_embedding_dim + 2)

    def forward(self, X, S, T):
        s = self.speaker_embedding(S)
        s = torch.squeeze(s, 2)
        T = F.normalize(T.float(), p=2, dim=1)
        h_t, h_n = self.rnn(X)
        #print(h_t.shape, s.shape, T.shape)
        sent_s = torch.cat([h_t, s, T], 2)
        a, v = self.sent_attn(sent_s)
        return a, v
